...
# @@File             :  docker.sh
# @@Description      :  Phase 2 binary validation inside a throwaway alpine container
...
__help() {
  ...
  printf '  Builds every binary in %s, then validates them inside a\n' "${CASHP_GO_IMAGE}"
  printf '  throwaway %s container with curl, bash, file and jq installed.\n' "${CASHP_TEST_IMAGE}"
  __help_section "Options"
  __help_line "--help" "Show this help message and exit"
  __help_line "--version" "Show version and exit"
  __help_line "--port 64580" "Port the test server listens on"
  ...
  __help_line "CASHP_TEST_PORT" "Same as --port (default: 64580)"
  ...
  __help_line "CASHP_TEST_IMAGE" "Image the binaries are exercised in (default: alpine:latest)"
  __help_line "CASHP_GO_IMAGE" "Toolchain image used to build (default: casjaysdev/go:latest)"
  __help_line "CASHP_STARTUP_TIMEOUT" "Seconds to wait for the server to answer (default: 60)"
  __help_line "GO_CACHE" "Host Go module cache to bind mount"
  __help_line "GO_BUILD" "Host Go build cache to bind mount"
  printf '\n'
}
# - - - - - - - - - - - - - - - - - - - - - - - - -
__version() {
...
__cleanup() {
  if [[ -n "${CONTAINER_NAME}" ]]; then
    if [[ -n "${TEMP_DIR}" ]] && [[ "${TESTS_FAILED}" -ne 0 ]]; then
      docker cp "${CONTAINER_NAME}:/tmp/server.log" "${TEMP_DIR}/server.log" &>/dev/null || true
    fi
  ...
  if [[ -n "${TEMP_DIR}" ]] && [[ -d "${TEMP_DIR}" ]]; then
    if [[ -s "${TEMP_DIR}/server.log" ]]; then
      printf 'Server log kept at %s/server.log\n' "${TEMP_DIR}"
    else
      rm -rf "${TEMP_DIR}"
    fi
  fi
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Random suffix keeps concurrent runs from colliding on a container name
__rand_suffix() {
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Start the alpine container, install the required tooling and stage the binaries
__start_container() {
  __section "Test container"
  CONTAINER_NAME="${PROJECT_NAME}-test-$(__rand_suffix)"
  printf 'Starting %s as %s...\n' "${CASHP_TEST_IMAGE}" "${CONTAINER_NAME}"
  docker run -d \
    --name "${CONTAINER_NAME}" \
    -v "${PROJECT_DIR}/binaries:/opt/binaries:ro" \
    -e "CASHP_BASE_URL=${BASE_URL}" \
    -e "CASHP_API_VERSION=${CASHP_API_VERSION}" \
    -e "CASHP_ADMIN_PATH=${CASHP_ADMIN_PATH}" \
    "${CASHP_TEST_IMAGE}" \
    sh -c 'sleep 3600' >/dev/null
  printf 'Installing curl bash file jq...\n'
  __in_container apk add --no-cache curl bash file jq >/dev/null
  __in_container sh -c 'mkdir -p /app && cp -a /opt/binaries/. /app/ && chmod 0755 /app/*'
  printf 'Copying test suites into the container...\n'
  docker cp "${SCRIPT_DIR}/test_endpoints.sh" "${CONTAINER_NAME}:/app/test_endpoints.sh"
  docker cp "${SCRIPT_DIR}/test_content_negotiation.sh" "${CONTAINER_NAME}:/app/test_content_negotiation.sh"
  docker cp "${SCRIPT_DIR}/test_admin_auth.sh" "${CONTAINER_NAME}:/app/test_admin_auth.sh"
  __in_container chmod 0755 /app/test_endpoints.sh /app/test_content_negotiation.sh /app/test_admin_auth.sh
}
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Every binary must answer --version and --help without a config file present
__check_binary_basics() {
  local _binary="$1" _label="$2"
  if __in_container "/app/${_binary}" --version >/dev/null 2>&1; then
    __pass "${_label} --version"
  else
    __fail "${_label} --version"
  fi
  if __in_container "/app/${_binary}" --help >/dev/null 2>&1; then
    __pass "${_label} --help"
  else
    __fail "${_label} --help"
  fi
  if __in_container file "/app/${_binary}" | grep -q -- 'ELF'; then
    __pass "${_label} is a static ELF binary"
  else
...
__check_binary_rename() {
  ...
  __in_container sh -c "cp /app/${_binary} /app/${_renamed} && chmod 0755 /app/${_renamed}"
  if __in_container sh -c "/app/${_renamed} --help 2>&1" | grep -q -- "${_renamed}"; then
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Launch the server inside the container and wait until it actually answers
__start_server() {
  local _waited=0
  __section "Server startup"
  docker exec -d "${CONTAINER_NAME}" sh -c "/app/${PROJECT_NAME} --port ${CASHP_TEST_PORT} >/tmp/server.log 2>&1"
  while [[ "${_waited}" -lt "${CASHP_STARTUP_TIMEOUT}" ]]; do
    if __in_container curl -q -sf -o /dev/null "${BASE_URL}${API_PREFIX}/server/healthz"; then
      __pass "server answered ${API_PREFIX}/server/healthz after ${_waited}s"
      return 0
    fi
    sleep 1
    _waited=$((_waited + 1))
  done
  __fail "server did not answer within ${CASHP_STARTUP_TIMEOUT}s"
  printf '%s--- server log ---%s\n' "${C_BOLD}" "${C_RESET}" >&2
  __in_container sh -c 'tail -n 50 /tmp/server.log' >&2 || true
  return 1
}
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Pull the first-run setup token out of the captured server log
__read_setup_token() {
  SETUP_TOKEN="$(__in_container sh -c "grep -i -- 'setup token' /tmp/server.log | grep -oE -- '[A-Za-z0-9_]{24,}' | tail -n1" 2>/dev/null || true)"
  if [[ -n "${SETUP_TOKEN}" ]]; then
    __pass "setup token captured from the server log (${SETUP_TOKEN:0:4}xxxxx)"
  else
    __fail "no setup token found in the server log"
  fi
}
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Run one of the standalone suites inside the container against the live server
__run_suite() {
  local _suite="$1"
  shift
  __section "Suite: ${_suite}"
  if __in_container env "$@" "/app/${_suite}" --color no; then
    __pass "suite ${_suite}"
  else
    __fail "suite ${_suite}"
  fi
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# The client binary must work against the live server, not merely print help
__test_client() {
  __section "Client binary"
  if ! __in_container test -f "/app/${PROJECT_NAME}-cli"; then
    printf 'client not built - skipping\n'
    return 0
  fi
  __check_binary_basics "${PROJECT_NAME}-cli" "client"
  __check_binary_rename "${PROJECT_NAME}-cli" "renamed-cli" "client"
  if [[ -n "${API_TOKEN}" ]]; then
    if __in_container "/app/${PROJECT_NAME}-cli" --server "${BASE_URL}" --token "${API_TOKEN}" status >/dev/null 2>&1; then
      __pass "client status with an API token"
    else
      __fail "client status with an API token"
    fi
  else
    if __in_container "/app/${PROJECT_NAME}-cli" --server "${BASE_URL}" status >/dev/null 2>&1; then
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# The agent binary must register against the live server using the issued API token
__test_agent() {
  __section "Agent binary"
  if ! __in_container test -f "/app/${PROJECT_NAME}-agent"; then
    printf 'agent not built - skipping\n'
    return 0
  fi
  ...
  if __in_container "/app/${PROJECT_NAME}-agent" --server "${BASE_URL}" --token "${API_TOKEN}" status >/dev/null 2>&1; then
...
__main() {
  ...
  TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/${PROJECT_ORG}/${PROJECT_NAME}-XXXXXX" 2>/dev/null || true)"
  if [[ -z "${TEMP_DIR}" ]]; then
    mkdir -p "${TMPDIR:-/tmp}/${PROJECT_ORG}"
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/${PROJECT_ORG}/${PROJECT_NAME}-XXXXXX")"
  fi
  printf '%s%s phase 2 docker validation (org: %s, port: %s)%s\n' "${C_BOLD}" "${PROJECT_NAME}" "${PROJECT_ORG}" "${CASHP_TEST_PORT}" "${C_RESET}"
  __build_binaries
  ...
  if ! __start_server; then
    printf '\n%sDocker validation FAILED - the server never came up%s\n' "${C_RED}" "${C_RESET}" >&2
    exit 1
  fi
  __read_setup_token
  ...
  if [[ -n "${SETUP_TOKEN}" ]]; then
    ...
    API_TOKEN="$(__in_container sh -c 'cat /tmp/api_token 2>/dev/null' || true)"
  fi
  __test_client
  __test_agent
  __section "Shutdown"
  __in_container sh -c "pkill -f '/app/${PROJECT_NAME}' || true"
  printf '\n%sTotal: %s  Failed: %s%s\n' "${C_BOLD}" "${TESTS_RUN}" "${TESTS_FAILED}" "${C_RESET}"
  if [[ "${TESTS_FAILED}" -ne 0 ]]; then
    printf '%sDocker validation FAILED%s\n' "${C_RED}" "${C_RESET}" >&2
    exit 1
  fi
  printf '%sDocker validation passed%s\n' "${C_GREEN}" "${C_RESET}"
}
# - - - - - - - - - - - - - - - - - - - - - - - - -
